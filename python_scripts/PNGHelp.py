...
def buffer_to_chunk(buffer : bytearray, chunk_type : str):
    chunk = bytearray()
    chunk += len(buffer).to_bytes(4, byteorder='big', signed=False)
    chunk += chunk_type + buffer + zlib.crc32(chunk_type + buffer).to_bytes(4, byteorder='big', signed=False)
    return chunk
...
def write_ihdr(png_data : bytearray, width : int, height : int, bpp : int, color_type : int):
    ...
    buffer += (color_type).to_bytes(1, byteorder='big', signed=False)
    buffer += (0).to_bytes(3, byteorder='big', signed=False)
...
def write_trns(png_data : bytearray, color_data : bytearray):
    buffer = bytearray()
    for i in range(0, len(color_data), 4):
        buffer += color_data[i + 3].to_bytes(1)
...
def write_text(png_data : bytearray, keyword : str, string : str):
    buffer = bytearray()
    buffer += bytearray(keyword, 'ascii') + (0).to_bytes(1, byteorder='big', signed=False)
